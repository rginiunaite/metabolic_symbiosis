...
% include the birth term which is increased in the presence of
% intracellular lactate (OXPHOS)
...
% population 1
    ...
    deriv(3) = yp(3) - ((birth(y(5),y(7)) - death(y(7),y(5))) * y (3));

% population 2

    deriv(4) = yp(4) - ((birth(y(6),y(8)) - death(y(8),y(6))) * y (4));
...
% intracellular lactate 
    ...
    deriv(7) = yp(7) - (Sl1 - VM4 * MCT41 * y(3)*y(7)/(KMAX4 + y(7)) + VM1*MCT11 * y(3)*y(1)/(KMAX1 +y(1)) - 0.005 * y(7)*y(3)); % 1st component
    
    deriv(8) = yp(8) - (Sl2 - VM4 * MCT42 * y(4)*y(8)/(KMAX4 + y(8)) + VM1*MCT12 * y(4)*y(2)/(KMAX1 +y(2)) - 0.005 * y(8)*y(4)); % 1st component
...
function value = birth (oxygen,lactate)
    ...
    oxycr = 0.02;
    nu0 = 5e-4;
   % value = a0 * ((oxygen/oxycr)-1 + (oxygen*lactate)/(oxycr*(0.001+lactate)))^exponent;
   
%     value = a0 * ((oxygen/oxycr)-1)^exponent;
%     value = 1/value + (1/a0)*(lactate/(1+lactate));
    
    
    %value = a0 * ((oxygen/oxycr)-1 + (oxygen*lactate)/(oxycr*(0.001+lactate)))^exponent;
    value = a0 * ((oxygen/oxycr)-1)^exponent;
    value = 1/value + (nu0)*(oxygen*lactate/(0.2*(1+lactate)));
    %value = 1/value;
