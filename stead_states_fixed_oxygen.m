...
% fixed oxygen
% note that here there is no source of lactate
...
IC(3) = 10;     % Initial population 1st compartment % similar to solution
IC(4) = 10;     % Initial population 2nd compartment % similar to solution
...
syms l1 l2 n1 n2 m41 m42

jac = jacobian([-omega12*l1 + omega21 * l2 + k1*n1* m41 / (KMAX4 + l1) -   k2*n1 * l1 * MCT11  / (KMAX1 + l1) ...
     - k5 * l1,(-omega21*l2 + omega12 * l1 + k1*n2* m42 / (KMAX4 + l2)-  k2*n2 * l2 * MCT12 / (KMAX1 + l2) - k5 * l2),...
      ((birth(oxygen1) - death(l1,oxygen1)) * n1),((birth(oxygen2) - death(l2,oxygen2)) * n2),...
      (k3 * hypoxia(oxygen1) /(H0 + hypoxia(oxygen1)) - k4 * m41),...
      (k3 * hypoxia(oxygen2) /(H0 + hypoxia(oxygen2)) - k4 * m42) ] , [l1 l2 n1 n2 m41 m42]);


M = subs(jac, [l1 l2 n1 n2 m41 m42], y); % evaluate at the steady state
...
%     omega21=0;
    ...
    value(1) = (-omega12*y(1) + omega21 * y(2) + k1*y(3)* y(5) / (KMAX4 + y(1)) - ...
        k2*y(3) * y(1) * MCT11  / (KMAX1 + y(1)) - k5 * y(1)); % assume m1 = 1
    

    % lactate 2      

    value(2) = (-omega21*y(2) + omega12 * y(1) + k1*y(4)* y(6) / (KMAX4 + y(2)) - ...
        k2*y(4) * y(2) * MCT12 / (KMAX1 + y(2)) - k5 * y(2));
        
     
    % population 1

    value(3) = ((birth(oxygen1) - death(y(1),oxygen1)) * y(3));

    % population 2

    value(4) = ((birth(oxygen2) - death(y(2),oxygen2)) * y(4));
