...
function val = pop1(c1,c2)
    ...
    k5=0.005*InitialPop; % lactate degradation 
     ...
     val = (-omega12 * lac(c1) + omega21 *lac(c2) - k5 * lac(c1) )/ (-mct4(c1)/(KMAX4 + lac(c1)) + lac(c1)/(KMAX1 +lac(c1)));  
end
...
function val = pop2(c1,c2)
    ...
    k5=0.005*InitialPop; % lactate degradation 
     ...
     val = (omega12 * lac(c1) - omega21 *lac(c2) - k5 * lac(c2) )/ (-mct4(c2)/(KMAX4 + lac(c2)) + lac(c2)/(KMAX1 +lac(c2)));  
end
