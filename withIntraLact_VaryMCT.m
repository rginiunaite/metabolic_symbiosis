...
% searching for the combination of transporters expression that maximises
% the cells in both populations (assumption: there can be max 1000 in one compartment)
...
range = 0.05:0.05:0.1;
l_range = length(range);

% create empty vectors of desirable size
m1 = zeros(l_range*l_range,1); 
m4 = zeros(l_range*l_range,1); 
norm = zeros(l_range*l_range,1); 
hyp = zeros(l_range*l_range,1); 
total = zeros(l_range*l_range,1); 

for MCT11 = 0.05:0.05:0.1
    for MCT42 = 0.05:0.05:0.1
        ...
        norm (i) = final_normoxic;
        hyp(i) = final_hypoxic;
        
        if norm(i)>1000
            norm(i) = 1000; %make sure it does not exceed 1000
        end
        
        if hyp(i)>1000
            hyp(i) = 1000; %make sure it does not exceed 1000
        end        
        
        total(i) = norm(i) + hyp(i);
    end
end

[value, index] = max(total); %find the index of maximum value

figure
plot(total,'LineWidth', 2)
hold on
plot(norm,'LineWidth',2)
plot(hyp,'LineWidth',2)
h_legend = legend('Total','1st component','2nd component')
set(h_legend,'FontSize',14)
xlabel('Different combinations of MCT1_1 and MCT4_2','FontSize',14)
ylabel('Population','FontSize',14)
title(['Population varying expression of transporters, maximum at MCT1_1 = ' num2str(m1(index)) ', MCT4_2 = ' num2str(m4(index))],'FontSize',14);
